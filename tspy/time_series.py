...
class TimeSeries(object):
    ...
    def from_array(cls, array, start_date, freq, var_desc):
        periods = len(array)
        dt_index = pd.date_range(start=start_date, periods=periods, freq=str(freq))
    ...
    def random_linear(cls, start_date, freq, periods, mu, sigma):
        rand_array = np.linspace(0, periods, num=periods, endpoint=False) + np.random.normal(mu, sigma, periods)
        dt_index = pd.date_range(start=start_date, periods=periods, freq=str(freq))
        series = pd.Series(rand_array, index=dt_index)
        return TimeSeries(series=series, freq=freq, var_desc='Random Linear')

    @classmethod
    def zeroes(cls, start_date, freq, periods):
        array = np.linspace(0, periods, num=periods, endpoint=False)
        dt_index = pd.date_range(start=start_date, periods=periods, freq=str(freq))
        series = pd.Series(array, index=dt_index)
        return TimeSeries(series=series, freq=freq, var_desc='Zeroes')
    ...
    def future_dt_index(self, n_periods: int) -> pd.DatetimeIndex:
        ...
        """
        dt_index = pd.DatetimeIndex(start=self.end_date, periods=n_periods, freq=str(self.freq))
